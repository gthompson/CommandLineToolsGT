...
/* Configuration file */
char *configfile   = "../bin/ampengfft.d";
...
int main (int argc,char **argv)
{
	...
	char dummy1[SFILELINE];                    /* string to do pattern matching using sscanf */
	char dummy2[SFILELINE];                    /* string to do pattern matching using sscanf */
	...
	if (argc == 2){
		...
		fprintf (stderr,"USAGE: ampengfft s-file\n");
		exit (1);
	}
	...
	if ((fp_sfile = fopen(c_sfilename,"r")) == NULL) {
		fprintf (stderr,"ERROR: ampengfft: Failed to open for reading %s\n",c_sfilename);
		exit (1);
	}
	...
	while (fgets(in_line,82,fp_sfile)){
		sscanf(in_line," VOLC M%s %s %s",dummy,dummy1,dummy2);
		/* printf("DUMMY \n",dummy2); */
		if (dummy2[0]=='A'){
			fprintf (stderr,"ERROR: ampengfft already run for this event: %s\n",c_sfilename);
			exit (1);
		}			
	...
	for (i=0; i<i_num_wavfiles; i++){
		...
		if ((in_buffer = read_seisan_record(fp_wav,swap_flag)) == NULL){
			fprintf (stderr,"ampengfft skipping headers\n");
			exit(1);
		}
		free(in_buffer);
		...
		for (i_chan=1; i_chan<=event_head.num_chan; i_chan++) {
			...
			if ( ret == 1){
				fprintf (stderr,"ampengfft can't read chan %d header\n",i_chan);
				exit(1);
			}
			if ( ret == 2){
				if (debug)
			...
			if (fread (&record_length, sizeof(long), 1, fp_wav) != 1) {
				fprintf(stderr, "ERROR: ampengfft: Can't read length\n");
				exit(1);
			}
			if (swap_flag)
			...
			if (record_length != chan_head.num_samp*sizeof(long)){
				fprintf(stderr, "ERROR: ampengfft: record has wrong length (%d not %d)\n",
				                 record_length,chan_head.num_samp);
				exit(1);
			}
			if (fread(&i_signal[1], sizeof(long), chan_head.num_samp, fp_wav) != chan_head.num_samp){ 
				fprintf(stderr, "ERROR: ampengfft: Can't read data\n");
				exit(1);
			}
			if (fread (&record_length, sizeof(long), 1, fp_wav) != 1) {
				fprintf(stderr, "ERROR: ampengfft: Can't read length (second time)\n");
				exit(1);
			}
			if (swap_flag)
	...
	if ((fp_sfile = fopen(c_sfilename,"w")) == NULL) {
		fprintf (stderr,"ERROR: ampengfft: Failed to open for writing %s\n",c_sfilename);
		exit (1);
	}
